...
char& sstring::operator[](int ind) {
	if (ind >= this->n - 1 || ind < 0) throw "incorrect index";
	return this->str[ind];
}
...
sstring& sstring::operator+=(const sstring& s) {
	...
	delete[] this->str;
	this->str = new char[this->n];
	for (int i = 0; i < this->n - s.n; i++) this->str[i] = temp.str[i];
	for (int i = this->n - s.n; i < this->n; i++) {
		this->str[i] = s.str[i - this->n + s.n];
	}
	return *this;
}
...
bool sstring::operator==(const sstring& s) const {
	...
	for (int i = 0; i < this->n; i++) if (this->str[i] != s.str[i]) return false;
	return true;
}
bool sstring::operator!=(const sstring& s)const {
...
bool sstring::operator>(const sstring& s)const {
	for (int i = 0; i < this->n && i < s.n; i++) if (this->str[i] > s.str[i]) return true;
...
sstring sstring::substr(int start, int end) const{
	if (start < 0 || start >= this->n - 1 || end <= start || end > this->n) throw "incorrect index";
	sstring s;
	s.n = end - start + 1;
	s.str = new char[s.n];
	for (int i = 0; i < s.n - 1; i++) {
		s.str[i] = this->str[start + i];
	}
	s.str[s.n - 1] = '\0';
	return s;
}
